...
axis_type = ds.get("axis_type", "time")
x_label = "Time (s)" if axis_type == "time" else "Excitation Wavelength (nm)"
...
with tab_kinetics:
    ...
    with col1:
        ...
        fig.update_layout(
            ...
            xaxis=dict(title=x_label, gridcolor="#eef1f5", linecolor="#d0d7e2", tickfont=dict(color="#5a6a7e")),
...
with tab_analysis:
    ...
    with st.expander("📊 Tool 1 — IR/IF Ratio (Rayleigh-Mie Scattering)", expanded=True):
        ...
        fig_irif.update_layout(
            ...
            xaxis=dict(title=x_label, gridcolor="#eef1f5", linecolor="#d0d7e2"),
...
def build_zip():
    ...
    with zipfile.ZipFile(buf, 'w') as z:
        ...
        # Save raw intensity
        raw_csv = intensity.to_csv().encode('utf-8')
        z.writestr("raw_intensity.csv", raw_csv)
...
if st.button("📦 Build ZIP Export"):
    zip_bytes = build_zip()
    st.download_button("⬇ Download ZIP", zip_bytes, "spectra_export.zip", "application/zip")
